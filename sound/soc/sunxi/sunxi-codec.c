...
static int gpio_pa_shutdown;
struct clk *codec_apbclk, *codec_pll2clk, *codec_moduleclk;

static volatile unsigned int capture_dmasrc;
static volatile unsigned int capture_dmadst;
static volatile unsigned int play_dmasrc;
static volatile unsigned int play_dmadst;

/* Structure/enum declaration */
struct codec_board_info {
...
#endif
	.dma_addr	= CODEC_BASEADDRESS + SUNXI_DAC_TXDATA,
};
...
#endif
	.dma_addr	= CODEC_BASEADDRESS + SUNXI_ADC_RXDATA,
};
...
/* Hardware definition playback device */
...
/* Hardware definition recording equipment */
 ...
 *
 *	info () function is used to obtain detailed information on the control,
 *	the function passed to it must populate the second parameter
			...
			struct snd_ctl_elem_info *uinfo)
{
	...
	uinfo->count = shift ==	rshift ? 1 : 2;	/* the info of element count */
 ...
 *	get () function is used to get the control's current value and return
 *	it to user space
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	unsigned int rshift = mc->rshift;
	int	max = mc->max;
	/* find last (most-significant) bit set for masking */
	...
	/* The obtained value is written to the snd_ctl_elem_value */
	if (invert) {
 ...
 *	@ucontrol:	control	element	information
 *
 *	put () is used to write values from user space, if the value is changed,
...
/*
 * Various settings of each register. There are three main implementation
 * functions.
 *	.info = snd_codec_info_volsw,
 *	.get = snd_codec_get_volsw,
 *	.put = snd_codec_put_volsw,
 * It should be noted that the only difference between sunxi and sun5i is the
 * Master Playback Volume
 */
static const struct snd_kcontrol_new sunxibc_dac[] = {
	/* FOR B C VERSION */
	CODEC_SINGLE("Master Playback Volume", SUNXI_DAC_ACTL, 0, 0x3f, 0),
	CODEC_SINGLE("Playback Switch", SUNXI_DAC_ACTL, 6, 1, 0),
	CODEC_SINGLE("Fm Volume", SUNXI_DAC_ACTL, 23, 7, 0),
	CODEC_SINGLE("Line Volume", SUNXI_DAC_ACTL, 26, 1, 0),
	CODEC_SINGLE("FmL Switch", SUNXI_DAC_ACTL, 17, 1, 0),
	CODEC_SINGLE("FmR Switch", SUNXI_DAC_ACTL, 16, 1, 0),
	CODEC_SINGLE("LineL Switch", SUNXI_DAC_ACTL, 19, 1, 0),
	CODEC_SINGLE("LineR Switch", SUNXI_DAC_ACTL, 18, 1, 0),
	...
	/* from bit 9 to bit 12 microphone mute */
	CODEC_SINGLE("Mic Input Mux", SUNXI_DAC_ACTL, 9, 15, 0),
};

static const struct snd_kcontrol_new sunxia_dac[] = {
	/* For A VERSION */
	CODEC_SINGLE("Master Playback Volume", SUNXI_DAC_DPC, 12, 0x3f, 0),
	CODEC_SINGLE("Playback Switch", SUNXI_DAC_ACTL, 6, 1, 0),
	CODEC_SINGLE("Fm Volume", SUNXI_DAC_ACTL, 23, 7, 0),
	CODEC_SINGLE("Line Volume", SUNXI_DAC_ACTL, 26, 1, 0),
	CODEC_SINGLE("FmL Switch", SUNXI_DAC_ACTL, 17, 1, 0),
...
static const struct snd_kcontrol_new codec_adc_controls[] = {
	...
	CODEC_SINGLE("MicL Volume", SUNXI_ADC_ACTL, 25, 3, 0),
	CODEC_SINGLE("MicR Volume", SUNXI_ADC_ACTL, 23, 3, 0),
	...
	CODEC_SINGLE("ADC Input Mux", SUNXI_ADC_ACTL, 17, 7, 0),
};
...
static const struct snd_kcontrol_new sun7i_adc_ctls[] = {
	 ...
	 * 000:-4.5db, 001:-3db, 010:-1.5db, 011:0db
...
{
	/*
	 *	Each alsa predefined components required in the constructor call
	 *	snd_device_new (), and each component's destructor method is
	 *	included in the function set.
	 *	For PCM, AC97 these predefined components, we do not care about
	 *	their destruction, and for custom components, you need to fill
	 *	snd_device_ops.
	 *	The destructor pointer dev_free, so that when snd_card_free ()
	 *	is called, the component will be automatically released.
	 */
	...
	int i, err;
	/*
	 *	snd_ctl_new1 function is used to create a snd_kcontrol and
	 *	returns its pointer,
	 *	snd_ctl_add snd_kcontrol function is used to create the card
	...
	if (codec_chip_ver == SUNXI_VER_A10A) {
		if (has_playback)
			for (i = 0; i < ARRAY_SIZE(sunxia_dac); i++) {
				err = snd_ctl_add(card,
					snd_ctl_new1(&sunxia_dac[i], clnt));
				if (err < 0)
					return err;
			}
		if (has_capture)
			for (i = 0; i < ARRAY_SIZE(codec_adc_controls); i++) {
				err = snd_ctl_add(card,
					snd_ctl_new1(&codec_adc_controls[i],
									clnt));
				if (err < 0)
					return err;
			}
	} else if (sunxi_is_sun5i() || codec_chip_ver == SUNXI_VER_A10B ||
					codec_chip_ver == SUNXI_VER_A10C) {
		if (has_playback)
			for (i = 0; i < ARRAY_SIZE(sunxibc_dac); i++) {
				err = snd_ctl_add(card,
					snd_ctl_new1(&sunxibc_dac[i], clnt));
				if (err < 0)
					return err;
			}
		if (has_capture)
		...
		if (has_playback)
			for (i = 0; i < ARRAY_SIZE(sun7i_dac_ctls); i++) {
				err = snd_ctl_add(card,
					snd_ctl_new1(&sun7i_dac_ctls[i], clnt));
				if (err < 0)
					return err;
			}
		if (has_capture)
			for (i = 0; i < ARRAY_SIZE(sun7i_adc_ctls); i++) {
				err = snd_ctl_add(card,
					snd_ctl_new1(&sun7i_adc_ctls[i], clnt));
				if (err < 0)
					return err;
			}
	} else {
	...
	/*
	*	When the card is created, the devices (components) can be
	*	created and associated with the card. The first parameter
	*	is snd_card_create. Creating a card pointer, the second
	*	parameter type refers to device-level device type that is
	*	in the form of SNDRV_DEV_XXX, including SNDRV_DEV_CODEC,
	*	SNDRV_DEV_CONTROL, SNDRV_DEV_PCM, SNDRV_DEV_RAWMIDI, etc.,
	*	user-defined Equipment, device-level is SNDRV_DEV_LOWLEVEL,
	*	ops argument is a set of functions (snd_device_ops structure)
	*	of Pointer, device_data is the device data pointer,
	*	snd_device_new itself does not allocate memory device data,
	*	should therefore advance Assignment. Here in snd_card_create
	*	assignment.
	*/
	err = snd_device_new(card, SNDRV_DEV_CODEC, clnt, &ops);
	if (err < 0)
		return err;
...
static void sunxi_pcm_enqueue(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		while (capture_prtd->dma_loaded < capture_limit) {
			if ((capture_pos + capture_len) >
						capture_prtd->dma_end)
				capture_len = capture_prtd->dma_end -
								capture_pos;
...
static snd_pcm_uframes_t snd_sunxi_codec_pointer(struct snd_pcm_substream
								*substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		play_res = play_dmasrc + play_prtd->dma_period -
							play_prtd->dma_start;
		spin_unlock(&play_prtd->lock);
		...
		sunxi_dma_getcurposition(capture_prtd->params,
						(dma_addr_t *)&capture_dmasrc,
						(dma_addr_t *)&capture_dmadst);
		capture_res = capture_dmadst + capture_prtd->dma_period -
...
static int sunxi_codec_pcm_hw_params(struct snd_pcm_substream *substream,
					struct snd_pcm_hw_params *params)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (play_prtd->params == NULL) {
			...
			if (play_ret < 0) {
				printk(KERN_ERR "failed to set dma callback. "\
						"ret == %d\n", play_ret);
			...
			play_prtd->dma_end = play_prtd->dma_start +
								play_totbytes;
		...
		if (capture_prtd->params == NULL) {
			...
			capture_ret =
				sunxi_dma_request(capture_prtd->params, 0);
			if (capture_ret < 0) {
				printk(KERN_ERR "failed to get dma channel. "\
						"capture_ret == %d\n",
						capture_ret);
			...
			capture_ret =
				sunxi_dma_set_callback(capture_prtd->params,
						sunxi_audio_capture_buffdone,
						substream);
			if (capture_ret < 0) {
				printk(KERN_ERR "failed to set dma callback. "\
						"capture_ret == %d\n",
			...
			capture_prtd->dma_limit =
						capture_runtime->hw.periods_min;
			...
			capture_prtd->dma_end = capture_prtd->dma_start +
							capture_totbytes;
...
static int snd_sunxi_codec_prepare(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		codec_play_dma_conf.xfer_type.src_data_width
							= DATA_WIDTH_16BIT;
		codec_play_dma_conf.xfer_type.src_bst_len = DATA_BRST_4;
		codec_play_dma_conf.xfer_type.dst_data_width
							= DATA_WIDTH_16BIT;
		codec_play_dma_conf.xfer_type.dst_bst_len = DATA_BRST_4;
		codec_play_dma_conf.address_type.src_addr_mode
							= NDMA_ADDR_INCREMENT;
		codec_play_dma_conf.address_type.dst_addr_mode
							= NDMA_ADDR_NOCHANGE;
...
#else
		memset(&codec_capture_dma_conf, 0,
						sizeof(codec_capture_dma_conf));
		codec_capture_dma_conf.xfer_type.src_data_width
							= DATA_WIDTH_16BIT;
		codec_capture_dma_conf.xfer_type.src_bst_len	= DATA_BRST_4;
		codec_capture_dma_conf.xfer_type.dst_data_width
							= DATA_WIDTH_16BIT;
		codec_capture_dma_conf.xfer_type.dst_bst_len	= DATA_BRST_4;
		codec_capture_dma_conf.address_type.src_addr_mode
							= NDMA_ADDR_NOCHANGE;
		codec_capture_dma_conf.address_type.dst_addr_mode
							= NDMA_ADDR_INCREMENT;
...
static int snd_sunxi_codec_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
			codec_capture_start();
			mdelay(1);
			codec_wr_control(SUNXI_ADC_FIFOC, 0x1,
							ADC_FIFO_FLUSH, 0x1);
...
{
	/* Run-time information to obtain PCM pointer */
	struct snd_pcm_runtime *runtime = substream->runtime;
	int err;
	struct sunxi_capture_runtime_data *capture_prtd;

	capture_prtd = kzalloc(sizeof(struct sunxi_capture_runtime_data),
								GFP_KERNEL);
...
static int snd_sunxicard_playback_open(struct snd_pcm_substream *substream)
{
	...
	play_prtd = kzalloc(sizeof(struct sunxi_playback_runtime_data),
								GFP_KERNEL);
	if (play_prtd == NULL)
		return -ENOMEM;
...
static struct snd_pcm_ops sunxi_pcm_playback_ops = {
	.open			= snd_sunxicard_playback_open,
	.close			= snd_sunxicard_playback_close,
	.ioctl			= snd_pcm_lib_ioctl,
	.hw_params		= sunxi_codec_pcm_hw_params,
	.hw_free		= snd_sunxi_codec_hw_free,
	.prepare		= snd_sunxi_codec_prepare,
	.trigger		= snd_sunxi_codec_trigger,
	.pointer		= snd_sunxi_codec_pointer,
};

static struct snd_pcm_ops sunxi_pcm_capture_ops = {
	.open			= snd_sunxicard_capture_open,
	.close			= snd_sunxicard_capture_close,
	.ioctl			= snd_pcm_lib_ioctl,
	.hw_params		= sunxi_codec_pcm_hw_params,
	.hw_free		= snd_sunxi_codec_hw_free,
	.prepare		= snd_sunxi_codec_prepare,
								...
								int device)
{
	...
	snd_pcm_lib_preallocate_pages_for_all(pcm, SNDRV_DMA_TYPE_DEV,
						...
						32 * 1024, 32 * 1024);
	/*
	 *	Set PCM operation, the first parameter is a pointer snd_pcm,
	 *	the first two parameters are SNDRV_PCM_STREAM_PLAYBACK
	 *	or SNDRV_ PCM_STREAM_CAPTURE and the third parameter is the PCM
	 *	operating structure snd_pcm_ops
	 */
	if (has_playback)
...
static int __devinit sunxi_codec_probe(struct platform_device *pdev)
{
	...
	ret = snd_card_create(0, "sunxi-codec", THIS_MODULE,
					sizeof(struct sunxi_codec), &card);
	if (ret != 0)
		return -ENOMEM;
	/* Remove from private_data allocated memory size */
	chip = card->private_data;
	/* Sound chip dedicated data */
	card->private_free = snd_sunxi_codec_free;
	chip->card = card;

	/*
	 *	mixer, registration control (mixer) Interface
	 *	Create a control to achieve at least snd_kcontrol_new in info(),
	 *	get() and put () member function of these three
	 */
	err = snd_chip_codec_mixer_new(card);
	if (err)
		goto nodev;

	/*
	 *	PCM, audio playback-related, registered PCM interface
	 */
	err = snd_card_sunxi_codec_pcm(chip, 0);
	if (err < 0)
		goto nodev;
	...
	if (clk_set_parent(codec_moduleclk, codec_pll2clk))
		printk("try to set parent of codec_moduleclk to codec_pll2clk "\
			"failed!\n");
	...
	/* codec address remap */
	db->codec_base_req = request_mem_region(db->codec_base_res->start,
							0x40, pdev->name);
	...
	if (resume_work_queue == NULL) {
		printk("[su4i-codec] try to create workqueue for codec "\
								"failed!\n");
		ret = -ENOMEM;
...
/*
 *	suspend state, first disable left and right channels, and then silence.
 *	Disable the pa (amplifier), disable headset, disable dac->pa and then
 *	finally disable DAC
 *	Sequence is required as noise my occur in suspend.
 */
static int snd_sunxi_codec_suspend(struct platform_device *pdev,
							pm_message_t state)
{
...
/*
 *	resume state, first unmute, re-enable DAC, enable L / R DAC, enable PA,
 *	enable headphone, enable dac to pa.
 *	Sequence is required as noise my occur.
 */
...
static struct resource sunxi_codec_resource[] = {
	[0] = {
		.start	= CODEC_BASEADDRESS,
		.end	= CODEC_BASEADDRESS + 0x40,
...
static int __init sunxi_codec_init(void)
{
	...
	ret = script_parser_fetch("audio_para", "playback_used",
							&has_playback, 1);
	...
	ret = script_parser_fetch("audio_para", "capture_used",
							&has_capture, 1);
