 ...
 * @regs			i2s register base
 * @ccmregs			ccm register base
 * @ioregs			io register base
 * @slave			0: master, 1: slave
 * @mono			0: stereo, 1: mono
 * @samp_fs			audio sample rate (unit in kHz)
 * @samp_res			16 bits, 20 bits , 24 bits, 32 bits)
 * @samp_format			audio sample format (0: standard I2S,
 *				1: left-justified, 2: right-justified, 3: pcm)
 * @ws_size			16 BCLK, 20 BCLK, 24 BCLK, 32 BCLK)
 * @mclk_rate			mclk frequency divide by fs(128fs, 192fs, 256fs,
 *				384fs, 512fs, 768fs)
 * @lrc_pol			LRC clock polarity (0: normal ,1: inverted)
 * @bclk_pol			BCLK polarity (0: normal, 1: inverted)
 * @pcm_txtype			PCM transmitter type (0: 16-bits linear mode,
 *				1: 8-bits linear mode, 2: u-law, 3: A-law)
 * @pcm_rxtype			PCM receiver type  (0: 16-bits linear mode,
 *				1: 8-bits linear mode, 2: u-law, 3: A-law)
 * @pcm_sw			PCM slot width (8: 8 bits, 16: 16 bits)
 * @pcm_sync_period		PCM sync period (16/32/64/128/256)
 * @pcm_sync_type		PCM sync symbol size (0: short sync,
 *				1: long sync)
 * @pcm_start_slot		PCM start slot index (1--4)
 * @pcm_lsb_first		0: MSB first, 1: LSB first
 * @pcm_ch_num			PCM channel number
 */
struct sunxi_i2s_info {
	void __iomem   *regs;
