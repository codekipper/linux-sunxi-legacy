...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if (dhd->dhd_cspec.ccode[0] == 0) {
			memcpy(dhd->dhd_cspec.country_abbrev, "KR", WLC_CNTRY_BUF_SZ);
			memcpy(dhd->dhd_cspec.ccode, "KR", WLC_CNTRY_BUF_SZ);
			dhd->dhd_cspec.rev = 3;
	}
	if (dhd->dhd_cspec.ccode[0] != 0) {
		...
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)) < 0) {
			printf("%s: country code setting failed and set to default country KR\n", __FUNCTION__);
			memcpy(dhd->dhd_cspec.country_abbrev, "KR", WLC_CNTRY_BUF_SZ);
			memcpy(dhd->dhd_cspec.ccode, "KR", WLC_CNTRY_BUF_SZ);
			dhd->dhd_cspec.rev = 3;
			bcm_mkiovar("country", (char *)&dhd->dhd_cspec,
