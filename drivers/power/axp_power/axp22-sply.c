...
void  set_aldo1(void)
{
	...
	ret = axp_read(axp_charger->master,AXP22_LDO_DC_EN1,&val);

	 if(ret)
          printk("====hanbiao set set_aldo1 ctrl  failure0!!!=============\n");
	 
	val |=  0x40;

	ret =axp_write(axp_charger->master, AXP22_LDO_DC_EN1, val);
	 if(ret)
          printk("====hanbiao set set_aldo1 ctrl  failure1!!!=============\n");
	 

	val= 0x1f;
	ret =axp_write(axp_charger->master, AXP22_ALDO1OUT_VOL, val);
	 if(ret){
          printk("====hanbiao set set_aldo1 ctrl  failure2!!!=============\n");
	 }

	  printk("====hanbiao set set_aldo1 ctrl  success!!!=============\n");

}
EXPORT_SYMBOL_GPL(set_aldo1);
#endif
...
void  set_dldo3(void)
{
	...
	ret = axp_read(axp_charger->master,0x12,&val);
	val |=  0x20;

	ret =axp_write(axp_charger->master, 0x12, val);

	val= 0x1f;
	axp_write(axp_charger->master, 0x17, val);

}
EXPORT_SYMBOL_GPL(set_dldo3);
#endif
