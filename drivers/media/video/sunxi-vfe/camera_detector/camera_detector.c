...
static __s32 tw2866_diff_i2c_id_detect(__camera_detector_t *camera_detector, 
                                                ...
                                                __u32 camera_list_size)
{
    ...
    detect_print("tw2866_diff_i2c_id_detect!!\n");

	    
    for (i = 0; i < camera_detector->num; i++) {
        for (j = 0; j < camera_list_size; j++) {
            if (camera_list[j].need_detect) {
				  detect_print("tw2866_diff_i2c_id_detect and the i == %d!!\n",i);
				   ///detect_print("tw2866_diff_i2c_id_detect dvdd + %d !!\n", (int)camera_detector->camera[i].dvdd));
				    detect_print("tw2866_diff_i2c_id_detect dvdd vol + %d !!\n", camera_detector->camera[i].dvdd_vol);
...
static int __init camera_detector_init(void) {
    ...
    || (camera_detector->num == 1)) {
              //hanbiao add only for tw2866
		tw2866_diff_i2c_id_detect(camera_detector, camera_list, camera_list_size);
    }
