...
#define TW2866_CSI_STBY_ON     1
#define TW2866_CSI_STBY_OFF    0
#define TW2866_CSI_RST_ON      0
#define TW2866_CSI_RST_OFF     1
#define TW2866_CSI_PWR_ON      1
#define TW2866_CSI_PWR_OFF     0
...
static void camera_pwr_on_tw2866(__u32 list_index, __camera_info_t *camera_info)
{
    //power on reset
	//camera_gpio_write(camera_info->stby_pin, camera_list[list_index].CSI_STBY_ON);
	
	//camera_gpio_write(camera_info->reset_pin, TW2866_CSI_RST_ON);
	///msleep(1);
	//active mclk before power on
	clk_enable(camera_info->module_clk);
	...
	//power supply
	///camera_gpio_write(camera_info->pwr_pin, TW2866_CSI_PWR_OFF);
	//msleep(20);
	///camera_gpio_write(camera_info->pwr_pin, TW2866_CSI_PWR_ON);


	 list_print("dvdd_vol000 is %s!! \n", camera_info->dvdd_str);

#if 0
	 if((!camera_info->dvdd)  && (strcm	p(camera_info->dvdd_str, "")))
	 {
	      list_print("get regulator csi_dvdd error!! is %s \n", camera_info->dvdd_str);
	 	camera_info->dvdd  = regulator_get(NULL, camera_info->dvdd_str);
        if (IS_ERR(camera_info->dvdd)) {
                list_print("get regulator csi_dvdd error!! \n");
                //return -EPMUPIN;
            }
	 }
#endif 
	...
	if(camera_info->dvdd) 
	{
	      list_print("dvdd_vol111 is %d!! \n", camera_info->dvdd_vol);
		 	
		if(camera_info->dvdd_vol > 0)
		{
			list_print("dvdd_vol222 is %d!! \n", camera_info->dvdd_vol);
			regulator_set_voltage(camera_info->dvdd,camera_info->dvdd_vol,camera_info->dvdd_vol);
		}
		else
		{
	...
	if(camera_info->avdd)
	{
		if(camera_info->avdd_vol > 0)
		{
			list_print("avdd_vol is %d!! \n", camera_info->avdd_vol);
			regulator_set_voltage(camera_info->avdd,camera_info->avdd_vol,camera_info->avdd_vol);
		}
		else
		{
			list_print("avdd_vol is NULL!! \n");
		}
		regulator_enable(camera_info->avdd);
		msleep(10);
	}
	if(camera_info->iovdd) 
	{
		if(camera_info->iovdd > 0)
		{
			list_print("iovdd_vol is %d!! \n", camera_info->iovdd_vol);
			regulator_set_voltage(camera_info->iovdd,camera_info->iovdd_vol,camera_info->iovdd_vol);
		}
		else
		{
			list_print("iovdd is NULL!! \n");
		}
		regulator_enable(camera_info->iovdd);
		msleep(10);
	}
	...
	//msleep(10);
	//reset after power on
	//camera_gpio_write(camera_info->reset_pin, TW2866_CSI_RST_OFF);
	//msleep(20);
}

static void camera_pwr_off_tw2866(__u32 list_index, __camera_info_t *camera_info)
{
#if 0
    //inactive mclk before power off
    clk_disable(camera_info->module_clk);
...
static __s32 camera_detect_tw2866(__u32 list_index, struct i2c_adapter *i2c_adap)
{
#if 0
	int ret;
	struct regval_list regs;
    ...
    list_print("detect camera_detect_tw2866!!\n");
...
__camera_list_t camera_list[MAX_CAMERA_LIST_ITEM] = {
    ...
    //CAMERA_LIST_ITEM_INIT(siv121d,       1, 1, 0x66, 1, 0, 0, 1, 1, 0),
     CAMERA_LIST_ITEM_INIT(tw2866,        1, 1, 0xB8, 1, 0, 0, 1, 1, 0), 
};
