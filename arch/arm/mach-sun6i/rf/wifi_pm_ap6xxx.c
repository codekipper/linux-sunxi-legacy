...
static char * axp_clk_name ="axp22_dldo4";
...
static int ap62xx_clk_power(int onoff)
{
	struct regulator* clk_ldo = NULL;
	int ret=0;
	
	clk_ldo = regulator_get(NULL, axp_clk_name);
	if (!clk_ldo) {
		ap6xxx_msg("get clock power regulator failed.\n");
		return -ret;
	}
	if(onoff)
	{
		ret = regulator_set_voltage(clk_ldo, 2800000, 2800000);
		if (ret < 0) {
			ap6xxx_msg("regulator_set_voltage fail, return %d.\n", ret);
			regulator_put(clk_ldo);
			return ret;
		}

		ret = regulator_enable(clk_ldo);
		if (ret < 0) {
			ap6xxx_msg("regulator_enable fail, return %d.\n", ret);
			regulator_put(clk_ldo);
			return ret;
